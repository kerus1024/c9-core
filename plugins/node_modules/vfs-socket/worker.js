...
function Worker(vfs) {
    ...
    function makeStreamProxy(token) {
        ...
        if (token.readable) stream.readable = token.readable;
        if (token.writable) stream.writable = token.writable;
    ...
    function storeStream(stream) {
        ...
        var token = {id: id};
        stream.token = token;
        if (stream.readable) token.readable = stream.readable;
        if (stream.writable) token.writable = stream.writable;
        return token;
    }
